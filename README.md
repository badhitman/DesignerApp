...
### Хранение файлов
Файлы, встречающиеся в системе могут быть в одном из трёх состояний:
...
- Вложения (локальное хранилище): вложения к заказам (пришедшие через api/rest). Хранятся локально в `MongoDB.Driver.GridFS`, но ни какой дополнительной информации о таких файлах не хранится. Об этих файлах известна только их связь с документом/владельцем (на логическом уровне СУБД). Такой файл считается внешним (по отношению к локальному хранилищу МетаФайлов)
- МетаФайлы (локальное хранилище)[^10]: файлы хранятся так же в `MongoDB.Driver.GridFS`, но о каждом таком файле в базе данных дополнительно существуют метаданные. Эти файлы не связаны с прикладными объектами системы на логическом уровне СУБД, но по их мета-данным можно определить что это за файл и к чему он относится. В метаданных два обязательных параметра `ApplicationName` и `PropertyName` и три не обязательных: `PrefixPropertyName`, `OwnerPrimaryKey`, `Referrer`. Кроме того: этим файлам можно назначать произвольные Теги.![files](./img/files-storage-base.png) и установить ограничения доступа к файлу[^10].

#### Логирование (Nlog)
Nlog пишет и в файлы и в базу данных. Строка подключения к БД в конфигах 'nlog.config'

### Настройка
- Перед запуском служб в Blazor[^4] потребуются конфиги **Email** (отправка писем **SMTP**. в. т.ч. для **Identity**), а в **TelegramBot** потребуется токен. **MQ** настройки потребуются всем сервисам. База данных может быть как SQLite, так и PostgreSQL/MySQL.
...
[^10]: МетаФайлы поддерживают контроль доступа. По умолчанию файлы доступны публично по ссылке, но можно установить ограничения на чтение. Правила доступа позволяют разрешать доступ персонально пользователям или по ассоциации с документом. В случае доступа по ассоциации с документом проверяется участие пользователя с документом (должен быть среди подписчиков или являться автором|исполнителем). Кроме того при создании каждому файлу генерируется токен доступа (guid), по которому можно получить доступ вне всяких ограничений. Если этот токен установить пустую строку или null - тогда доступ по токену становится невозможным
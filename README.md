...
### ToolsMauiApp
Клиент для управления службами удалённо: колхоз на тему ci/cd. На стоковом хостинге организовать CI/CD полного цикла не получается: 1-2 ядра и 1-2 GB ОЗУ не хватает для сборки (dotnet publish). Идея в том, что бы публиковать на рабочем ПК, оценить разность файлов публикаций и доставить на сервер только минимально необходимые данные для приведения удалённой папки к равенству локальной. Сверка идёт по имени, размеру и версии файла.

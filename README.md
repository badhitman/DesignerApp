...
> [!NOTE]
> constructor - гибкое решение для конструирования документов непосредственно в web (на данный момент на этапе разработки). А так же генерация C# .net кода для созданной структуры сущностей. Такое решение позволит с нуля сконструировать новое приложение со своей структурой БД и CRUD+ набором сервисов. Этот сгенерированный код легко ляжет на данную базовую заготовку. Таким образом станет возможно непосредственно через WEB/UI сконструировать приложение любого масштаба, после чего можно сосредоточиться на доработках сгенерированного/шаблонного кода под свои задачи.

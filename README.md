...
![logs](./img/tools-logs-filters.png)
...
- Локальная сборка: [build-win.ps1](https://github.com/badhitman/DesignerApp/blob/main/devops/build-win.ps1) (скрипт **ОЧИСТИТ ПАПКУ НАЗНАЧЕНИЯ** и соберёт серверные службы в неё `C:\Users\User\Documents\publish`). Для тестирования сборку следует запускать с ключом `-c Debug`![build debug](./img/build-debug.png)

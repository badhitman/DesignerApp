...
- Telegram cloud: Если в Telegram сообщениях фигурируют вложения (файлы), ссылки на скачивание из TG сохраняются в локальной базе данных. При попытке скачать этот файл происходит online скачивание данных с серверов TG. Эти данные не сохраняются ни где и доступны только онлайн из TG.
- Вложения (локальное хранилище): вложения к заказам (пришедшие через api/rest). Хранятся локально в MongoDB.Driver.GridFS, но ни какой информации об этих файлов не хранится - только связь с документом-владельцем.
- МетаФайлы (локальное хранилище): файлы хранятся так же локально в MongoDB.Driver.GridFS, но о каждом таком файле хранятся метаданные в базе данных. Например при добавлении файлов в статью (cms), об этом файле будет сохранена информация о связи с прикладным объектом системы. Кроме того этим файлам можно назначать Теги.

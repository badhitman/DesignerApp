...
- **Blazor NET.9**[^4] + **TelegramBot**[^5]: подойдёт как стартовый кейс web решения с поддержкой **Telegram бота**. Поддержка автоматической авторизации в Telegram WebApp.
...
Если разрешить автоматическое подключение API Telegram, тогда решение, запущенное в TelegramBot как WebApp будет автоматически авторизовываться клиента. Каждое посещение каждой страницы проверяется необходимость авторизации через Телеграм и выполняет вход в систему самостоятельно без ввода логина и/или пароля.

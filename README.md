...
- решение по создания форм и документов.
...
- Telegram cloud: если в Telegram сообщениях фигурируют вложения (файлы), ссылки на скачивание из TG сохраняются в базе данных. При попытке пользователем скачать такой файл происходит online скачивание данных с серверов TG. Эти данные не сохраняются ни где в системе, а доступны только режиме онлайн из TG.
- Вложения (локальное хранилище): вложения к заказам (пришедшие через api/rest). Хранятся локально в `MongoDB.Driver.GridFS`, но ни какой дополнительной информации о таких файлах не хранится. Об этих файлах известна только их связь документом/владельцем.
- МетаФайлы (локальное хранилище): файлы хранятся так же в `MongoDB.Driver.GridFS`, но о каждом таком файле в базе данных дополнительно существуют метаданные. Эти файлы не связаны с прикладными объектами системы на уровне СУБД, но по их мета-данным можно определить что это за файл и к чему он относится. В метаданных два обязательных параметра `ApplicationName` и `PropertyName` и три не обязательных: `PrefixPropertyName`, `OwnerPrimaryKey`, `Referrer`. Кроме того: этим файлам можно назначать произвольные Теги.![files](./img/files-storage-base.png)

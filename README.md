## Blazor NET.9 + Telegram Bot + СЭД (HelpDesk/ServiceDesk)

- **Blazor NET.9**[^4] + **TelegramBot**[^5]: подойдёт как стартовый кейс web решения с поддержкой **Telegram бота**.
...
- Подсистема электронной коммерции: на базе СЭД функционирует простой учёт заказов в т.ч. остатки на складах.
...
Для функционирования [утилиты (win/android)](https://github.com/badhitman/DesignerApp?tab=readme-ov-file#tools-maui-app) в части синхронизации папок с целью доставки сборок на сервер (через штатный rest/api сервис) потребуется предусмотреть подходящий максимальный размер тела запроса. Он должен быть не меньше 10 мегабайт. На данный момент самый большой файл Blazor билда около 9 мегабайт в сжатом состоянии (каждый файл перед отправкой сжимается).
- Конфигурацию можно проверить/изменить тут: `/etc/nginx/nginx.conf`
- Параметр (рекомендуемый на данный момент): `client_max_body_size 10M;`
- Применить изменения (перезагрузить службу): `systemctl restart nginx`
...
- На сервере `ApiRestService` в настройках требуется указать максимальный размер пакета данных `PartUploadSessionConfig:PartUploadSize` и время жизни сессии загрузки файлов `PartUploadSessionConfig:PartUploadSessionTimeoutSeconds`. Размер пакета установлен по умолчанию 2 мегабайта - такой размер предположительно должен пройти через любые ограничения (Nginx, Kestrel и т.п.), но в случае необходимости можно его изменить. Этот параметр используется для нарезки отправляемых файлов если они больше этого ограничения. Перед отправкой файла клиент запрашивает сессию для загрузки файла. Сервер создаёт в Redis сессию и сообщает клиенту как нужно нарезать (какими частями) файл. Клиент подчиняется инструкциям сервера. Куски (части) файла в рамках одной сессии временно помещаются в Redis на срок указанной вами срока сессии (по умолчанию 1 минута). Если в рамках одной сессии не успеть отправить все части файла, тогда сессия со всеми доставленными частями исчезает и дозагрузка становится невозможной.

### STAGE контур
Запуск нескольких копий решения внутри единой инфраструктуры. Можно на одной общей инфраструктуре (Redis, Mongo, Postgres, RabbitMQ) запустить несколько копий решения таким образом что бы они друг другу не мешали. Для этого нужно указать/придумать префикс нового контура (имя) и предусмотреть под это имя строки подключения к СУБД и Redis. RabbitMQ автоматически по имени контура будет добавлять префиксы к именам очередей, а Mongo используется только для хранилища файлов. Я не настаиваю на том что подобная практика рекомендуемая, но иногда такой подход может потребоваться в целях тестирования.

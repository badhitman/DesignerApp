...
- Вложения (локальное хранилище): вложения к заказам (пришедшие через api/rest). Хранятся локально в `MongoDB.Driver.GridFS`, но ни какой дополнительной информации о таких файлах не хранится. Об этих файлах известна только их связь документом/владельцем (на логическом уровне СУБД).
- МетаФайлы (локальное хранилище)[^10]: файлы хранятся так же в `MongoDB.Driver.GridFS`, но о каждом таком файле в базе данных дополнительно существуют метаданные. Эти файлы не связаны с прикладными объектами системы на логическом уровне СУБД, но по их мета-данным можно определить что это за файл и к чему он относится. В метаданных два обязательных параметра `ApplicationName` и `PropertyName` и три не обязательных: `PrefixPropertyName`, `OwnerPrimaryKey`, `Referrer`. Кроме того: этим файлам можно назначать произвольные Теги.![files](./img/files-storage-base.png).
...
[^10]: МетаФайлы поддерживают контроль доступа. По умолчанию файлы доступны публично по ссылке, но можно установить ограничения. Правила доступа позволяют разрешать доступ персонально пользователям или по ассоциации с документом. В случае доступа по ассоциации с документом проверяется участие пользователя с документом (должен быть среди подписчиков или являться автором|исполнителем) 
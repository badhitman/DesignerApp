...
### Tools (maui app)
[Утилита для удалённого взаимодействия с сервисами](https://github.com/badhitman/DesignerApp/tree/main/ToolsMauiApp) Windows/Android. Работает через штатный [rest/api](https://github.com/badhitman/DesignerApp/tree/main/ApiRestService):
- Колхоз на тему ci/cd. Стоковый хостинг не в силах выдержать CI/CD полного цикла (1-2 ядра и 1-2 GB ОЗУ не хватает для сборки). Идея в том, что бы собираться на рабочем ПК и доставить на сервер только те файлы, которые изменились/появились, а лишние удалять. Сверка файлов производится по имени, размеру и md5:hash файла.

...
```
Для юридически значимых полей (Name, LegalAddress, INN, KPP, OGRN, CurrentAccount, CorrespondentAccount, BankName, BankBIC) есть дополнительное/резервное поле с префиксом `new`. Например есть поле `inn` и `newINN` (по остальным так же). При создании нового контрагента оба поля будут заполнены одним и тем же значением. Если клиент запросил изменения в реквизитах существующей компании, тогда в `inn` будет текущее/рабочее значение, а в поле `newINN` будет новое значение, которое клиент хочет установить. Т.е. клиент не может просто изменить реквизиты - может только запросить это изменение, а бухгалтер должен их принять или отклонить. Загрузка заказов по таким контрагентам (у которых есть не закрытый запрос изменений) может привести к выписке не корректных документов в последствии. По каждому такому объекту нужно принять решение и сбросить состояние в стандартное (когда в полях с префиксом `new` будет пусто). Для сброса состояния юрлица нужно воспользоваться следующим методом:
```
curl -X 'POST' \
...
```
У документа есть поле `addressesTabs`, которое представляют перечень адресов доставки по данному заказу. У каждого адреса доставки есть поле `rows` в котором хранятся строки/данные по номенклатуре заказа. 
